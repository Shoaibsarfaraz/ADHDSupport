...
export const Productivity = () => {
  ...
  // --- New State for Brain Dump ---
  const [brainDumpInput, setBrainDumpInput] = useState('');

  // --- New State for Focus Timer ---
  ...
  const handleAddTask = (e: React.FormEvent) => {
    ...
    const updatedUser = {
      ...user,
      plannerEntries: [...user.plannerEntries, newEntry],
      updatedAt: new Date(),
    };
  ...
  const handleToggleTask = (entryId: string | undefined) => {
    ...
    const updatedEntries = user.plannerEntries.map(entry =>
      entry._id === entryId
        ? { ...entry, pEntryStatus: entry.pEntryStatus === 'completed' ? 'pending' : 'completed' as const }
        : entry
    );
    setUser({ ...user, plannerEntries: updatedEntries, updatedAt: new Date() });
  };
  ...
  const handleDeleteTask = (entryId: string | undefined) => {
    ...
    setUser({
      ...user,
      plannerEntries: user.plannerEntries.filter(e => e._id !== entryId),
      updatedAt: new Date(),
    });
  };

  // --- Mood Handlers (Existing) ---
  const handleAddMoodCheckin = (e: React.FormEvent) => {
    ...
    setUser({ ...user, emotionalCheckins: [...user.emotionalCheckins, newCheckin], updatedAt: new Date() });
    setMoodNotes('');
  };

  // --- Brain Dump Handlers (New) ---
  const handleAddBrainDump = (e: React.FormEvent) => {
    ...
    // Assuming user.brainDumpEntries exists
    const updatedUser = {
      ...user,
      brainDumpEntries: [newDump, ...(user.brainDumpEntries || [])], 
      updatedAt: new Date(),
    };
  ...
  const handleDeleteBrainDump = (id: string) => {
    ...
    setUser({
      ...user,
      brainDumpEntries: user.brainDumpEntries.filter(b => b._id !== id),
      updatedAt: new Date(),
    });
  };

  // --- Focus Timer Handlers (New) ---
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      if (isWorkMode && user) {
        ...
        const newSession: IFocusSession = {
            _id: `focus_${Date.now()}`,
            duration: 25, // Assuming standard pomodoro for now
            completedAt: new Date()
        };
        setUser({
            ...user,
            focusSessions: [...(user.focusSessions || []), newSession],
            updatedAt: new Date()
        });
        // Switch to break
  ...
  }, [isActive, timeLeft, isWorkMode, user, setUser]);
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="space-y-3 max-h-[600px] overflow-y-auto">
                  {!user?.brainDumpEntries || user.brainDumpEntries.length === 0 ? (
                    <p className="text-[#263A47] text-center py-8">Mind full? Pour it out here.</p>
                  ) : (
                    user.brainDumpEntries.map(entry => (
            ...
            <div className="bg-white rounded-lg shadow-md p-8 text-center">
              ...
              <div className="border-t border-[#D7E9ED] pt-6">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                   ...
                   {(!user?.focusSessions || user.focusSessions.filter(s => new Date(s.completedAt).toDateString() === new Date().toDateString()).length === 0) && (
